"""Repositório MongoDB para a Nota de Saída, com adaptadores que espelham
a interface do modelo SQLAlchemy (NotaSaida/ItemNota), para que rotas e
templates funcionem sem alterações, seja qual for a origem dos dados.

Estado embutido no próprio documento (padrão MongoDB): os itens e o
histórico ficam dentro do documento da nota (arrays), não em coleções
à parte — evita joins e mantém a nota inteira numa só leitura.
"""
...
class _ItemMongoAdapter:
    """Espelha app.models.item.ItemNota a partir de um sub-documento."""
...
class _EventoHistoricoMongo:
    """Espelha app.models.historico.Historico a partir de um item do array embutido."""
...
class _NotaMongoAdapter:
    ...
    def __init__(self, doc: dict):
        ...
        self.criador = _PessoaRefMongo(self.criado_por, doc.get("criador_username"), doc.get("criador_nome"))
        self.aprovador = _PessoaRefMongo(self.aprovado_por, doc.get("aprovador_username"), doc.get("aprovador_nome"))
        self.revisao_tecnico = _PessoaRefMongo(
    ...
    @property
    def historico(self):
        return [_EventoHistoricoMongo(d) for d in sorted(
            self._historico_docs, key=lambda d: d.get("data_hora") or _to_utc(datetime.min), reverse=True
        )]
    ...
    # -- regras de negócio (idênticas a NotaSaida) --------------------------
    ...
    def __repr__(self):
        return f"<_NotaMongoAdapter {self.numero_referencia} ({self.estado})>"
...
class MongoNotaRepository:
    ...
    def criar(self, *, numero_referencia, data_emissao, funcionario, email_funcionario,
              ...
              itens=None, estado=None) -> _NotaMongoAdapter:
    ...
    def obter_por_id(self, nota_id) -> _NotaMongoAdapter | None:
        doc = self.col.find_one({"_id": _as_object_id(nota_id)})
        return _NotaMongoAdapter(doc) if doc else None

    def listar(self, filtro: dict | None = None) -> list[_NotaMongoAdapter]:
        return [_NotaMongoAdapter(d) for d in self.col.find(filtro or {}).sort("data_criacao", -1)]

    def contar(self, filtro: dict | None = None) -> int:
        return self.col.count_documents(filtro or {})

    # -- escrita ---------------------------------------------------------
    def _set(self, nota_id, campos: dict) -> _NotaMongoAdapter:
    ...
    def atualizar_dados(self, nota_id, *, numero_referencia, data_emissao, funcionario,
                         ...
                         origem_local="Sede IT", local_emissao="Maputo", itens=None) -> _NotaMongoAdapter:
        campos = {
            "numero_referencia": (numero_referencia or "").strip(),
            "data_emissao": _to_utc(data_emissao),
            "funcionario": funcionario.strip(),
    ...
    def definir_assinatura(self, nota_id, papel: str, *, path=None, posicao=None, limpar=False):
        """Define (ou limpa) o path/posição de um papel de assinatura."""
        if limpar:
            self.col.update_one(
    ...
    def dashboard(self, criado_por=None) -> tuple[dict, list[_NotaMongoAdapter]]:
        ...
        recentes = [_NotaMongoAdapter(d) for d in self.col.find(filtro).sort("data_criacao", -1).limit(7)]
